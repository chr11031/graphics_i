class Matrix4x4
{
	...
	set_perspective(fov_lr_deg, fov_td_deg, near, far)
	{
		...
		this.set(0, 1, 0.0);
		this.set(0, 2, (-near * (right + left)) / (right - left));
		this.set(0, 3, 0.0);

		this.set(1, 0, 0.0);
		this.set(1, 1, 2.0 * near / (top - bot));
		this.set(1, 2, (-near * (top + bot)) / (top - bot));
		this.set(1, 3, 0.0);
...
function load_Matrix_4x4_from_columns_array(arr)
{
	var rv = new Matrix4x4();
	...
	{
		console.log(arr);
		alert('Invalid array size for generating 4x4 matrix');
		return null;
	}
...
function _length_r3(V)
{
	var len = Math.sqrt( V[0]*V[0] + V[1]*V[1] + V[2]*V[2] );
	return len;
}

function _normalize_r3(V)
{
	var len = _length_r3(V);
...
function _dot_r3(A, B)
{
	return A[0]*B[0] + A[1]*B[1] + A[2]*B[2];
}

function _vec3_pos_to_vec4(vec3)
{
	var vec4 = new Float32Array([vec3[0], vec3[1], vec3[2], 1.0]);
	return vec4;
}

function _vec3_dir_to_vec4(vec3)
{
	var vec4 = new Float32Array([vec3[0], vec3[1], vec3[2], 0.0]);
	return vec4;
}

function _vec4_pos_to_vec3(vec4)
{
	var vec3 = new Float32Array([vec4[0] / vec4[3], vec4[1] / vec4[3], vec4[2] / vec4[3]]);
	return vec3;
}

function _vec4_dir_to_vec3(vec4)
{
	var vec3 = new Float32Array([vec4[0], vec4[1], vec4[2]]);
	return vec3;
}

function _arbitrary_up_vec3_from_look_vec3(look3, pos3)
{
	var vec = _normalize_r3(look3);

	var vec_a = new Float32Array([0.0, 0.0, 0.0]);
	if (vec[0] == 0.0)
	{
		vec_a = new Float32Array([1.0, 0.0, 0.0]);		
	}
	else if (vec[1] == 0.0)
	{
		vec_a = new Float32Array([0.0, 1.0, 0.0]);
	}
	else if (vec[2] == 0.0)
	{
		vec_a = new Float32Array([0.0, 0.0, 1.0]);
	}
	else 
	{
		vec_a = new Float32Array([1.0, vec[0]/vec[1], -2*vec[0] / vec[2]]);
		vec_a = _normalize_r3(vec_a);
	}

	var vec_b = _cross_r3( vec_a, vec );
	return vec_b;
}
