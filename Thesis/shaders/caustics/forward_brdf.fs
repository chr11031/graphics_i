...
vec3 shade()
{
	vec3 albedo       	= texture(albedo_map, uv_interp).rgb;
	float metallic	  	= texture(metal_map, uv_interp).r;
	float roughness		= texture(rough_map, uv_interp).g;
	float ao	  		= texture(ao_map, uv_interp).b;
