...
{	
	...
	{
		if (this._active_program == null)
		{
			throw new Error("No program in use - cannot complete draw call");
		}
	...
	_input_matches_source(input_type, src_size)
	{
		if (input_type == this.GLSL_VEC3 && src_size == 3)
		{
			return true;
		}
		else if (input_type == this.GLSL_MAT4 && src_size == 16)
		{
			return true; 
		}
		else
		{
			throw new Error("Unsupported type and size mismatch: " + input_type + ", " + src_size);
		}
	...
	_input_assembler(offset, num_vertices)
	{
		...
		// Construct vertex shader input data 
		var input_data_template = {};
		var attrib_indices = [];
		
		var keys = Object.keys(this._active_program.input_data);		
		for (var i = 0; i < keys.length; i++)
		{
			input_data_template[keys[i]] = null;			
			
			for (var p = 0; p < active_vao.attrib_ptrs.length; p++)
			{
				if (active_vao.attrib_ptrs[p].index == i && 
					_input_matches_source(this._active_program.input_data[keys[i]].type, active_vao.attrib_ptrs[p].size) && 
					i in active_vao.enable_attribs)
				{
					attrib_indices.push(p);
				}
			}

			
			if (input_data_template.length != attrib_indices.length)
			{
				throw new Error("Could not find vertex shader input in VAO layout");
			}
		}
		...
		{
			var vertex_input_data = structuredClone( input_data_template );
			...
			input_data.push( vertex_input_data );
